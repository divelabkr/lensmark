...
export const FEATURES: Feature[] = [
  ...
  },
  {
    ...
    files: ["src/lansmark/account/types.ts", "src/lansmark/account/accountStore.ts", "src/lansmark/account/sessionStore.ts", "src/lansmark/account/verifier.ts", "src/lansmark/notify/emailSender.ts", "server/routes/account.ts"],
    tests: ["src/lansmark/tests/accountRoutes.spec.ts", "src/lansmark/tests/emailMagicLink.spec.ts"],
    guardrails: ["원 식별자(전화/이메일) 미저장(authRef.subjectHash=keyed-hash)", "세션 토큰=무작위 192bit·만료", "OTP/이메일·운영+발송키없음 fail-closed(코드/링크 비노출)", "챌린지당 시도 상한(brute-force)·매직링크 256bit 1회용·타이밍세이프 비교", "매직링크 토큰 URL 즉시 제거(잔류 금지)", "auth는 sensitive 레이트리밋", "이관은 로그인 세션 필수"], status: "platform",
    notes: "휴대폰 OTP + 이메일 매직링크 병행(M2) — CompositeVerifier(challengeId 'method:' 프리픽스 라우팅). 실발송은 LiveSmsSender/LiveEmailSender 드롭인(HUMAN GATE: SMS 게이트웨이 키 + 동의화면 위탁 고지 / 이메일 제공자 키 + LANSMARK_APP_ORIGIN). 카카오는 같은 AuthVerifier 인터페이스로 추가. 유료 모드의 결제-계정 연계는 후속.",
  },
  {
