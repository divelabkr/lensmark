...
flowchart LR
  ...
  assess --> land
  recommend["🌱 무료 작물추천·전환<br/>작물→지역 추천(기후 적합)<br/>무료 작물추천<br/>지원금·지자체·농협 혜택<br/>기후 근거(필지 기후 프로필)<br/>작물 전환 로드맵(온난화 재결정)"]
  land --> recommend
  ...
  simulate --> growth
  operate["🌾 재배 운영·동반(기록·모니터링·알람·출하)<br/>재배 기록·시즌 리포트<br/>일일 환경 모니터링·시각화<br/>병충해·재난 알람<br/>알림 구독(opt-in 핸드폰)<br/>웹푸시 알림(앱 푸시·SMS 대체)<br/>출하 시세·납품처 최적화"]
  growth --> operate
  ...
  subgraph PLATFORM["⚙ 플랫폼·운영 (cross-cutting)"]
    ...
    client_error_telemetry["클라이언트 에러 텔레메트리 + 실시간 경보"]
...
| **웹푸시 알림(앱 푸시·SMS 대체)** | 알림 모달 '브라우저 알림 받기' → /api/push/vapid(configured?) → 권한 요청 → SW pushManager.subscribe → /api/push/subscribe(구독 저장). 발송 시 SW push 이벤트가 알림 표시·클릭 시 앱 포커스. | `/api/push/vapid` `/api/push/subscribe` `/api/push/unsubscribe` | 1 | 1 | 🟢 live | 구독 저장 + SW 표시/클릭 다리 live · 실제 발송(LiveWebPushSender: VAPID JWT ES256 + aes128gcm)·VAPID 키 생성=HUMAN GATE(integrations/push.ts) · 구독 영속(File store)=follow-up · SMS(alert-subscribe) 대신 무과금 앱 푸시 채널(사용자 선택) |
...
| 기능 | 흐름 | 엔드포인트 | 파일 | 테스트 | 상태 | 비고/seam |
|---|---|---|--:|--:|---|---|
| **실측 보정 플라이휠(해자)** | 실측 제출(유료게이트) → 작물·지형버킷 보정 → 다음 예측 현실화 → validated(서로 다른 제출자 5↑) | `/api/feedback` | 4 | 3 | 🟢 live | ★ B2C→B2B 다리: B2C 사용(일지 수확·실측 제출)이 작물·지역버킷 보정을 쌓아 demo를 실측으로 대체 → validated 누적이 곧 B2B(객관 근거 판매)의 전환 게이트. B2C 단계의 '실측 제출 인센티브'가 해자·B2B의 연료. |
...
| **클라이언트 에러 텔레메트리 + 실시간 경보** | 사용자 브라우저의 uncaught JS 에러/거부를 POST /api/client-error로 수집(이전엔 0=사장님이 못 봄). 집계·디듀프·최근 링버퍼·PII 0·distinct 상한. '새 distinct' 에러만 활동로그 + 웹훅(LANSMARK_ALERT_WEBHOOK · Slack/Discord) 실시간 경보. /api/ops/stats.clientErrors로 콘솔 '서버' 탭 + watch 종합판정에 노출. opsWatch가 distinct≥1 warn·≥5 crit. | `/api/client-error` | 2 | 1 | 🟢 live | 웹훅 미설정이면 조용히(기록은 됨). 메모리 보관(재시작 휘발 — 텔레메트리엔 충분). 프론트 리포터=dashboard/lansmark_app.html(세션 상한 8·디듀프·keepalive). |
...
---
범례: 🟢 live · 🟡 mock · 🟠 seam(키/스펙 대기) · ⚙ platform. 총 **39** 기능 · 단계 9.
