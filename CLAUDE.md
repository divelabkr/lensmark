...
## 🤖 로컬 qwen 1차 리뷰 (무료 게이트 · 선택적 가속)
> 도구: `/Users/yongj/Documents/Playground/qwen`(로컬 `qwen3-coder:30b`·Ollama, 토큰 0·오프라인·프라이빗). 규칙·역할은 그곳 `AGENTS.md`/`CLAUDE.md`. ⚠ LENSMARK은 git repo가 아니라 **파일 기반** 리뷰를 쓴다.
- **언제**: 슬라이스 그린(tsc·vitest·arch) 후, 내 멀티에이전트 red-team **전에** 무료 1차 거름망으로.
- **방법**: `~/Documents/Playground/qwen/scripts/review-files <변경/신규 파일…>` → 한국어 5섹션(치명/테스트누락/보안·개인정보/품질/승인전확인). **구현+테스트+관련 파일을 함께 넣어야 정확**(부분 입력은 오탐·미관측 다발 — 실증됨).
- **삼각검증(필수)**: qwen은 **1차 보조이지 최종 판단이 아니다**(AGENTS.md). 결과를 *확정 결함* vs *불확실*로 나눠 — **확정만 수정**, 불확실은 Claude 판단/사람 승인. 자동 커밋·푸시·배포·삭제·시크릿 변경 금지.
- **비용 분담**: 저위험 슬라이스 = qwen 1차로 충분 / **고위험(보안·결제·해자) = qwen 1차 + Claude 멀티에이전트 적대 red-team**(토큰 절약). 잡일(드래프트·대량변환·요약)도 로컬 qwen에 오프로드 가능(Mode 2).
