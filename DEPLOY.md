...
```
사용자 → https://lensmark.kr → [Firebase Hosting: TLS·CDN·도메인] → (rewrite **) → [Cloud Run: lansmark-api]
...
```bash
gcloud run deploy lansmark-api \
  --source . \
...
```
- `--source .` → Cloud Build가 `Dockerfile`로 빌드. **서비스명·리전은 `firebase.json` rewrite·`scripts/deploy.sh`(SSOT)와 반드시 일치**(`lansmark-api`/`asia-northeast3`). ⚠ 철자 주의: 서비스는 **LAN**(`lansmark-api`) — 도메인 `lensmark.kr`(LEN)·프로젝트 `lensmark-dev`(LEN)과 다르다. 불일치 시 Hosting이 Cloud Run을 못 찾아 사이트 전체가 연결 실패.
...
# → https://lensmark-dev.web.app (모든 경로가 Cloud Run lansmark-api로 rewrite)
```
> ⚠ **`firebase.json`(rewrite serviceId 포함) 변경은 이 명령으로 재배포해야 라이브에 반영**된다 — `scripts/deploy.sh`의 `gcloud run deploy`는 Cloud Run만 갱신하고 Hosting 설정은 건드리지 않는다. (배경: serviceId 오타 수정이 레포엔 있어도 hosting 미배포면 사이트가 계속 연결 실패.) `bash scripts/deploy.sh hosting`으로도 실행 가능.
...
gcloud run services update lansmark-api --region asia-northeast3 --update-env-vars LANSMARK_STORE=firestore \
...
gcloud run services update lansmark-api --region asia-northeast3 --update-secrets \
...
- 발송 키 전까지 로그인/알림은 dev 표시·fail-closed. 롤백: `gcloud run services update-traffic lansmark-api --to-revisions PREV=100`.
