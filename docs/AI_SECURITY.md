...
## 현 상태 한 줄
LLM seam은 **휴면**(라우트 미연결·키 없음·`verified:false`). 그러나 켜질 때를 대비해 **숫자 날조 방어는 이미 다중방어**로 구현됨. 아래는 대조표.
...
| **LLM04 모델 DoS(비용폭증)** | 호출 폭주로 토큰 비용 폭증 | 캐시(TTL 24h·negTTL·cap 500) + 15s 타임아웃 + max_tokens 400. **라우트 연결 시**: sensitive 레이트리밋 버킷 + 결제 게이팅 필요 | 🟡 연결 시 게이팅 TODO |
...
1. [ ] 라이브 키로 **실응답 1건 캡처** → 출력가드(`hasUnprovidedMoney`/`hasFabricatedUrl`) 보정 후 `verified:true` 승격.
2. [ ] 엔드포인트를 **sensitive 레이트리밋 버킷**에 등록(`SENSITIVE_RE`) + (유료기능이면) 엔티틀먼트 게이팅.
3. [ ] 프론트에 **AI·비PII·보장아님** 고지 + 데이터가 Anthropic에 전송됨을 명시.
4. [ ] 인젝션 레드팀 1회(지역명에 주입 문자열 삽입 → 출력에 거짓 숫자/링크/지시이탈 없는지).
