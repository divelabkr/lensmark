...
## 현 상태 한 줄
LLM seam은 **verified(2026-06-19 승격)** — 라이브 실응답 캡처 + 출력가드(만/억 환산 정규화) 보정 + 인젝션 레드팀 3/3 통과. 라우트 `POST /api/explain` 배선·유료게이트+sensitive 레이트리밋·키 없으면 `explanation:null` 무중단·**UI는 배포·데이터 뒤(미노출)**. 숫자 날조 방어는 **다중방어** 구현됨. 아래는 대조표.
...
| **LLM02 안전하지 않은 출력 처리** | LLM 출력이 사용자에게 노출 | 출력 후처리 이중: `hasUnprovidedMoney`(만/억/천 단위를 **원으로 정규화** 후 엔진값과 대조 — 미제공 금액=폐기) + `hasFabricatedUrl`(링크=폐기). 프론트는 텍스트로만 렌더(HTML 주입 아님)·하드 라벨("AI 설명·숫자는 엔진·보장 아님") | 🟢 fail-closed |
...
## 라우트 연결 시 체크리스트 (verified 승격 — 2026-06-19 완료)
1. [x] 라이브 키로 **실응답 1건 캡처**(2026-06-19) → 출력가드 보정(만/억 환산을 원으로 정규화: `hasUnprovidedMoney` — "9,999만 원" 공백형 날조 갭 보강) 후 `verified:true` 승격. 도구: `scripts/explainSmoke.ts`.
2. [x] 엔드포인트를 **sensitive 레이트리밋 버킷**에 등록(`SENSITIVE_RE`) + 엔티틀먼트 게이팅 — `server/routes/explain.ts`·`middleware.ts` 완료.
3. [~] **AI·비PII·보장아님** 고지 — 응답에 `label`/`disclosure` 동봉(준비 완료). 프론트 렌더는 UI 노출(배포·데이터 뒤) 시 — **이 항목만 미완**(UI 보류, CLAUDE.md 원칙).
4. [x] 인젝션 레드팀 1회 — region에 주입(role헤더·"무시하고 99,999만 원"·"10억원 보장") → 출력에 거짓 숫자/보장/지시이탈 없음 확인(**3/3 PASS**). 도구: `scripts/explainRedteam.ts`.
...
- `hasUnprovidedMoney`는 단일 단위(억/만/천) 환산까지 원으로 정규화해 대조하지만 **복합 표현("1억 2천만 원")은 부분 매치**라 완전치 않음 — 단, 엔진값(백만원대 단순 환산)을 벗어난 복합 금액은 대부분 부분 매치로도 폐기되고, 핵심 위협("N만 원" 공백형 날조)은 닫혔다. 라이브 레드팀(3/3 PASS)으로 실효 확인. 더 견고히 하려면 프롬프트에 '원 단위 정수 표기' 강제 검토(자연스러움 트레이드오프).
