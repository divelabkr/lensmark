...
const SENSITIVE_RE = /^\/api\/(simulate|explain|feedback|pay\/|pg\/|geocode|parcel|terrain|journal|market|monitor|foreign|retail-price|briefing|account\/auth|account\/link|account\/register|account\/login|alerts|push\/(?:un)?subscribe|client-error|client-diag)/; // client-error·client-diag=공개 텔레메트리(비콘 플러드 차단) · retail-price·alerts=외부 API 쿼터 보호(레드팀 L3) · explain=LLM 비용폭증 차단(LLM04)
