...
const SENSITIVE_RE = /^\/api\/(simulate|explain|feedback|pay\/|pg\/|geocode|parcel|terrain|journal|market|monitor|foreign|retail-price|account\/auth|account\/link|alerts|push\/(?:un)?subscribe|client-error)/; // client-error=공개 텔레메트리 · retail-price·alerts=외부 API 쿼터 보호(레드팀 L3) · explain=LLM 비용폭증 차단(LLM04)
