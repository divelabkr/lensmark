...
export const RELEASES: Release[] = [
  {
    version: "0.82.0",
    date: "2026-07-02",
    title: "💳 유료 파일럿 준비 — 알림 영속·결제 전 정직 고지·세션 하드닝",
    items: [
      "아침 브리핑 알림 구독이 서버 재시작·재배포에도 유지됩니다(영속 저장 승격) — 알림 약속이 끊기지 않습니다.",
      "결제 전 페이월에서 선택 작물의 소득 데이터 상태를 미리 알려드립니다 — 💰 농진청 실측 기반 vs ⚠ 추정 구조값(참고용). 결제 후 실망이 없도록.",
      "로그인 보안 강화: 브라우저에는 세션을 안전한 쿠키로만 전달합니다(응답 본문 비노출).",
      "가격·상품 문서를 실제 구현(₩4,900·7일·재계산 50회 파일럿)과 일치시켰습니다.",
    ],
  },
  {
    version: "0.81.0",
    date: "2026-07-01",
    title: "⛰ 추천이 진짜 '내 땅'에 반응 — 지형 반영 + 💰 소득 실데이터 배지",
    items: [
      "무료 작물 추천이 필지의 실측 지형(경사·향, 위성 표고 실데이터)을 반영합니다 — 급경사 땅에선 급경사에 약한 작물의 순위가 실제로 내려갑니다(유료 시뮬과 동일 기준).",
      "추천 칩에 💰 배지 — 농진청 실측 소득자료(2024)가 있는 작물을 정직하게 구분합니다. 배지 없는 작물의 소득 숫자는 추정 구조값입니다.",
      "추천 근거에 경사·향 문구 추가(예: '경사 15° > 작물 허용(~10°) — 작업·토양유실 부담').",
    ],
  },
  {
    version: "0.80.0",
    date: "2026-07-01",
    title: "🔔 아침 브리핑 웹푸시 — 매일 여는 이유(발송 실구현)",
    items: [
      "내 농장 브리핑을 브라우저 알림으로 받습니다 — 브리핑 홈의 '아침 브리핑 알림 받기' 버튼으로 구독(명시 동의).",
      "웹푸시 발송을 표준(RFC 8291/8292)대로 자체 구현하고 공식 테스트 벡터로 검증 — 별도 앱 설치·SMS 과금 없이 무료.",
      "알림 제목은 가장 급한 것 하나만(기상특보 > 위험 경계 > 오늘의 브리핑) · 소득 수치는 알림에 싣지 않습니다.",
      "운영: VAPID 키 자가생성(npm run vapid:gen) 후 아침마다 발송 트리거(/api/ops/push-briefing·관리자) — 스케줄러 연결 예정.",
    ],
  },
  {
    version: "0.79.0",
    date: "2026-07-01",
    title: "🌾 오늘의 내 농장 — 데일리 브리핑(리텐션 피벗 1호)",
    items: [
      "재배를 시작한 땅(재배일지)이 '내 농장'이 됩니다 — 앱을 열면 지도 대신 오늘의 브리핑이 먼저: 오늘·7일 날씨, 서리·폭염·호우·강풍·건조 위험, 오늘 할 일, 생육 단계, 병해충·기상특보, 도매 시세.",
      "예보는 필지 좌표별 실데이터(Open-Meteo·무키) — 위험 알림은 작물 요구조건(룰북)과 매칭한 참고 정보이며 보장이 아닙니다(면책·출처 표기).",
      "로고(홈) 클릭 = 내 농장 브리핑(농장이 없으면 기존 작물 추천 루프).",
      "무료 추천 호출의 하드코딩 면적(3,300㎡) 제거 — 적합도 랭킹은 면적 무관, 소득 시뮬은 필지 실면적 사용(정직성).",
    ],
  },
  {
    version: "0.78.9",
  ...
  },
  {
    ...
    items: [
      "온보딩 '작물 추천' → 시장 트렌드 정렬 표(순서·작물·난이도·트렌드·단가·차별화). 헤더 클릭 정렬(기본 트렌드 높은 순)·색막대 게이지·작물 누르면 적합지(시도 기후 녹/적) 지도 → 정밀 시뮬.",
      "정직성: 트렌드·차별화=AI 시장조사(Perplexity·출처 동반·보장 아님)·단가=농진청 base 실값·난이도=재배 요구조건 룰(LLM 아님). 19작물 화이트리스트·출처0개·키없음 시 '땅 먼저' 폴백. ⚠ 실 Perplexity 응답 파서 보정은 라이브 후.",
    ],
  },
  {
    version: "0.77.16",
