...
describe("account routes — 휴대폰 OTP 가입·세션·익명→계정 이관(코어)", () => {
  ...
  it("브라우저 요청(Origin 동봉) → 바디에 세션 비노출·httpOnly 쿠키만(§3-1 하드닝)", async () => {
    const s1 = mockRes();
    await route(ctx, mockReq("POST", { origin: "https://lensmark.kr" }, { method: "phone", contact: "01099990000" }), s1, U("/api/account/auth/start"));
    const { challengeId, devHint } = JSON.parse(s1.captured.body);
    const s2 = mockRes();
    await route(ctx, mockReq("POST", { origin: "https://lensmark.kr" }, { challengeId, code: devHint }), s2, U("/api/account/auth/verify"));
    const v = JSON.parse(s2.captured.body);
    expect(v.ok).toBe(true);
    expect(v.session).toBeUndefined();                                 // 로그인 순간 XSS의 바디 판독 경로 차단
    expect(s2.captured.headers["set-cookie"]).toContain("HttpOnly");   // 세션은 쿠키로만
    expect(v.accountId).toBeTruthy();
  });

  it("익명 모드(anonOnly) → 계정 라우트 404(PII 미수집·PII-0 배포)", async () => {
    const anonCtx = createContext({ ...loadConfig(), requireEntitlement: false, anonOnly: true });
    const s = mockRes();
    await route(anonCtx, mockReq("POST", {}, { method: "phone", contact: "010-1111-2222" }), s, U("/api/account/auth/start"));
    expect(s.captured.code).toBe(404); // 익명 모드: 가입/로그인 비활성(이메일·전화 미수집)
  });
  ...
  it("이메일 매직링크 로그인(병행): start(email) → devHint 링크 → verify(token) → 세션·methods=email", async () => {
    const s1 = mockRes();
    await route(ctx, mockReq("POST", {}, { method: "email", contact: "Farmer@Example.com" }), s1, U("/api/account/auth/start"));
    expect(s1.captured.code).toBe(200);
    const { challengeId, devHint } = JSON.parse(s1.captured.body);
    expect(challengeId.startsWith("email:")).toBe(true);
    expect(devHint).toMatch(/^\/app\?lm_login=email:[0-9a-f]+~[0-9a-f]{64}$/); // dev 링크 경로
    const lm = new URL("http://localhost" + devHint).searchParams.get("lm_login")!;
    const token = lm.slice(lm.indexOf("~") + 1);
    const s2 = mockRes();
    await route(ctx, mockReq("POST", {}, { challengeId, code: token }), s2, U("/api/account/auth/verify"));
    expect(s2.captured.code).toBe(200);
    const v = JSON.parse(s2.captured.body);
    expect(v.session).toBeTruthy();
    expect(v.isNew).toBe(true);
    const me = mockRes();
    await route(ctx, mockReq("GET", { "x-lansmark-session": v.session }), me, U("/api/account/me"));
    expect(JSON.parse(me.captured.body).methods).toContain("email");
  });

  it("잘못된 이메일은 400(BAD_EMAIL)", async () => {
    const s1 = mockRes();
    await route(ctx, mockReq("POST", {}, { method: "email", contact: "not-an-email" }), s1, U("/api/account/auth/start"));
    expect(s1.captured.code).toBe(400);
    expect(JSON.parse(s1.captured.body).code).toBe("BAD_EMAIL");
  });
  ...
  it("결속(boundAccount): 결속 계정만 연결·타 계정은 403 (레드팀 #3 bearer 선점 차단)", async () => {
    const a = await login("01077776666");
    const b = await login("01077775555");
    const tokA = mintEntitlementToken({ userId: "order:BA", jti: "ent-bound-a", boundAccount: a.accountId });
    const okA = mockRes();
    await route(ctx, mockReq("POST", { "x-lansmark-session": a.session, "x-lansmark-entitlement": tokA }), okA, U("/api/account/link-entitlement"));
    expect(okA.captured.code).toBe(200); // 결속 계정 본인 → OK
    const tokA2 = mintEntitlementToken({ userId: "order:BA2", jti: "ent-bound-a2", boundAccount: a.accountId });
    const denyB = mockRes();
    await route(ctx, mockReq("POST", { "x-lansmark-session": b.session, "x-lansmark-entitlement": tokA2 }), denyB, U("/api/account/link-entitlement"));
    expect(denyB.captured.code).toBe(403); // 타 계정 결속 토큰 → 거부
    expect(JSON.parse(denyB.captured.body).code).toBe("ENTITLEMENT_BOUND_OTHER");
  });
