...
export interface ConfirmInput { paymentKey: string; orderId: string; amount: number; expectedAmount: number; userId: string; secretKey: string; ttlMs?: number; }
...
export async function confirmPayment(i: ConfirmInput): Promise<{ entitlementToken: string; orderId: string }> {
  ...
  const token = mintEntitlementToken({ userId: i.userId, source: "order", reference: i.orderId, jti: orderJti(i.orderId), exp: Date.now() + (i.ttlMs ?? 30 * 86400000) });
