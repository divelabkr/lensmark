...
## 0.82.0 — 2026-07-02 · 💳 유료 파일럿 준비 — 알림 영속·결제 전 정직 고지·세션 하드닝
> 유료 파일럿의 최소 조건 4종: 약속(알림)이 안 끊기고, 결제 전에 정직하고, 과금 전 보안 잔여를 지우고, 가격 문서가 구현과 일치.
- **푸시 구독 영속 승격** — `pushSubs`를 stores 소속으로(memory|file|firestore 3모드 어댑터 + FILE_STORE_FILES/STORE_DOC_IDS 등록=백업/복구 자동 포함 + firestore 워밍 대열 합류). 이전엔 메모리 휘발이라 재배포 1회에 전 구독자 아침 알림이 조용히 중단됐다(유료 약속 위반 리스크).
- **결제 전 정직 고지(페이월)** — 선택 작물의 소득 데이터 검증 상태를 결제 버튼 위에 표시: 💰 농진청 실측(2024) 기반 vs ⚠ 추정 구조값(절대 금액 참고용·💰 작물 안내). 미검증 작물에 결제 후 실망(환불 분쟁) 사전 차단.
- **세션 하드닝(§3-1 '과금 전 필수' 해소)** — verify/register/login 3곳: Origin 헤더 동봉(=브라우저) 요청엔 바디 `{session}` 생략·httpOnly Set-Cookie만(sessionBody 헬퍼). 로그인 순간 XSS의 바디 판독 경로 차단. 비브라우저 API/테스트는 기존 유지 + 회귀 테스트.
- **MONETIZATION.md 현행화** — 문서(19,000~299,000원 구상)와 구현(₩4,900·7일·50회) 불일치 해소: 파일럿 단일 상품=현행, 사다리=검증 후 로드맵으로 구분 + 파일럿 측정 가설 3종 명시.
- 테스트: 푸시 구독 재시작 보존(db.spec+1)·브라우저 바디 비노출(accountRoutes+1) → vitest 720.
...
- **일일 예보 provider(신규)** — Open-Meteo Forecast(무료·무키)·30분 TTL·~1km 격자 캐시·형태가드(부분 오염 거부)·실패 시 mock 폴백에 `demo` 라벨 강제(정직성). `LANSMARK_DATA_MODE=mock`이면 외부호출 0(테스트 결정성). ⚠ 비상업 티어 — 상업 전환 시 KMA 단기예보 교체 seam.
- **앱 홈 개편** — 내 농장이 있으면 첫 화면=오늘의 브리핑(카드: 오늘 기온·강수·바람 / 위험 배지 / 오늘 할 일 / 단계·D+경과·수확월 / 시세 / 주간 스트립 / 지도에서 열기). 로고=홈도 브리핑 우선. '재배 시작' 버튼을 "내 농장으로 등록"으로 재프레임(깔때기).
- **정직성 퀵윈** — 무료 추천 호출의 하드코딩 면적(3,300㎡) 제거(적합도 랭킹은 면적 무관·시뮬은 폴리곤 실면적).
- 신규 테스트: forecast(7)·dailyBriefing(12)·briefingRoutes(5) → vitest 698 · featureMap `daily-briefing` 등록(arch 그린).

## 0.77.16 — 2026-06-23 · 랜드라인 가독성 — 필지 경계선 강조
> 사용자 '토지 구분 명확·랜드 라인 잘 보이게' — 필지 경계선을 굵게·진하게.
- 선택 필지 경계 `weight 2→3.5`·진하게(opacity 1)·실선, 미리보기 2.5. 토지 윤곽 또렷.
- 지목 색은 landClass group 5종(농경지/물/도시/임야/기타)이라 4구역 다 '농경지' 초록 — 세부 지목별 색·추천 반영은 다음 작업(추천 로직이 지목을 안 보는 것 확인).
- CACHE v9→v10. tsc·vitest 659·arch·size 그린.

## 0.77.15 — 2026-06-23 · 버전 정합 — 옛 클라이언트 자동 갱신 안내(fail-open·차단 없음)
> 옛 버전 HTML이 SW 캐시에 갇히던 문제(먹통의 한 축)를 "안내 + 비파괴 자동 갱신"으로. 사용자 제안 중 '차단'은 의도적 먹통 위험으로 제외.
- **빌드 버전 주입** 서버 sendHtml이 `window.__BUILD_VER`(빌드 버전) 주입 → 클라가 `/api/version`(서버 최신)과 비교해 **stale(옛 캐시 HTML) 감지**.
- **stale 시** ① 배너 "새 버전 · 새로고침" ② SW 갱신 트리거 ③ 작업 중(입력·모달·결제) 아닐 때 **idle 1회 자동 새로고침**(무한루프 방지: sessionStorage·버전당 1회).
- **⚠ fail-open** 버전을 못 가져오면(콜드스타트·네트워크) stale 판정 안 함·**앱 정상 동작·차단 없음**. '옛 버전이면 못 켜게'는 버전 체크 실패 시 멀쩡한 앱도 막아(의도적 먹통) 채택 안 함.
- CACHE v8→v9. tsc·vitest 659·arch·size 그린.

## 0.77.14 — 2026-06-23 · 부팅 비콘 재시도 — 콜드스타트 관측 누락 방지
> 0.77.13 자동 관측을 라이브 실브라우저로 검증하다 발견·수정. route는 정상(204)인데 부팅 비콘만 콜드스타트 윈도우에서 503.
- **발견**: client-diag route는 curl·수동 fetch 모두 204인데, **부팅 시점 자동 비콘만 503** — min=0 콜드스타트 순간에 걸려 누락(정작 관측 대상인 콜드스타트를 놓치는 아이러니).
- **수정**: 비콘에 5xx/네트워크 실패 시 **0.9s 후 1회 재시도**(app.html) → 인스턴스 깬 뒤 관측 도달. CACHE v7→v8. 관측만·복구 아님(불변식 유지).

## 0.77.13 — 2026-06-23 · 에러 자동 관측 구조 — '사용자 설명 의존' 탈피(관측·추적·가이드 / 복구 없음)
> 이번 세션 내내 "사장님이 '먹통' 스크린샷 → 디버깅"이던 패턴을 "앱이 자동 관측 → 운영자 추적 → 가이드"로 전환.
- **관측** 앱 부팅 시 자기 환경(SW상태·**직전 오프라인**·콜드스타트·뷰포트·캐시버전)을 POST `/api/client-diag`로 익명 자동 보고. `window.onerror`로 안 잡히던 '안 뜨는 상태'(SW갇힘·OFFLINE_HTML·콜드스타트)를 관측. **OFFLINE_HTML이 localStorage 플래그를 남겨 다음 정상 로드가 '먹통'조차 자동 보고.**
- **추적** 서버 집계(오프라인율·SW분포·캐시버전 잔존) → ops '사용자 환경 진단' 패널 + 임계 초과 시 가이드 웹훅.
- **가이드** "무엇을 점검하라" 텍스트(예: 오프라인 다발 → CF sw.js 캐시·콜드스타트 점검).
- **⚠ 불변식: 복구 권한 없음** — SW unregister·캐시삭제·재시작은 절대 자동 안 함(관측/집계만·read·append). clientDiag store + 회귀가드 5종(복구 없음 명시 검증). CACHE v6→v7. tsc·vitest 659·arch·size 그린.

## 0.77.12 — 2026-06-23 · 운영 가시성 보강 — 데이터 신선도(마지막 live)·응답 p95·업타임
> "파이프라인 묶임·품질 점수·운영자 가시성" 점검의 공통 빈틈 = 신선도 시각·인프라 지연. 그 한 축을 채움.
- **신선도** provider 런타임 건강에 `lastLiveAt`(마지막 live 성공 시각·결정성 위해 at 주입식) → `/api/health`·ops 통합목록에 **"↳N시간 전"** 노출. "키 있고 live지만 며칠째 폴백 중"인 소스를 시각으로 드러냄.
- **지연·업타임** API 응답시간 측정(미들웨어 `res.finish`·최근 200건 링버퍼) → ops 시스템건전성에 **응답 p95**(800/2000ms=주의/시급)·**업타임**(min=0 재시작 빈도) 노출. 이전엔 5xx만 보여 "느려짐"이 사각.
- 데이터 패스 묶임·품질 게이트(A~D)·ops 콘솔(7탭)은 이미 탄탄 — 빠진 신선도·지연만 채움. runtimeHealth 결정성 유지(at 주입식). tsc·vitest 654·arch·size 그린.
...
- **OP-2** ops 유료게이트 켤 때 'PAID' 타이핑 확인(무료베타 중 페이월 사고 차단).
- **OP-3** 클라 에러 — 새 distinct + 같은 에러 50회 배수 '볼륨 폭증'도 경보(스팸 없이).
- tsc·vitest 622·arch·size 그린.

## 0.77.4 — 2026-06-23 · 사용자 흐름 개선(즉시구간) — 클릭 피드백·내위치 유도·시트 힌트·입력 점진공개
> 종합 UX 분석의 즉시구간. 팝업 모달화는 보류(map-first·모바일 바텀시트 충돌), 클릭 피드백은 스크롤+하이라이트로.
- **UX-1** 작물 클릭→소득카드 스크롤+펄스 하이라이트 · **UX-2** 빈 상태 '📍 내 위치로 바로' CTA(최대 이탈점) · **UX-5** 모바일 시트 핸들 펄스 힌트.
- **EL-1** 시뮬입력 점진공개 — 비율·면적·판로 기본 + 나머지 8개 ⚙고급 접기(인지부하↓·자동값 충분).
- tsc·vitest 621·arch·size + preview 검증(렌더·구문0). prefers-reduced-motion 존중.

## 0.77.3 — 2026-06-22 · 작물 재배 비율 슬라이더 — 일부만 농사지을 때 소득 비례
> 시뮬 입력에 '재배 비율'(전체 필지의 X%) 슬라이더 → 면적 스케일로 소득 비례. 작기·소득·병충해·시뮬입력은 기존 패널에 이미 제공 중.
- **재배 비율** — 전체 필지 면적 중 X%만 재배 시 면적 스케일(fullArea 기준)·면적칸 동기화 → 소득 비례. 땅 전부에 농사 못 짓는 현실 반영.
- tsc·vitest 621·arch·size 그린 + 로컬 preview 검증(비율 렌더·구문 에러 0·값 60%/600㎡).
...
- **AI 설명 캐시 버킷** — 키의 P50 정확수치→규모 버킷(100만/500만/2천만)으로 미스 제거. hit 시 현재 금액 정합 재확인(옛 설명 노출 차단·1원칙).
- tsc·vitest 621·arch·size 그린.
...
## 0.76.5 — 2026-06-17 · 설계 감사 즉시조치 ① — 무료추천↔유료시뮬 근거 일치 + 배포 철자 가드
> 감사에서 드러난 핵심 모순(무료 추천이 기후 무시 ↔ 유료 시뮬은 기후 반영) 봉합.
- **모순 제거** — 무료 추천도 유료와 동일 데이터기준(내한·내서성·물요구·서리민감)으로 기후 반영. /api/recommend가 기후를 먼저 가져와 랭킹+근거+시뮬 한 기준. 기후 위험 근거 우선 노출.
- **철자 가드** — deploy.sh가 firebase.json serviceId==SERVICE 확인 후 배포(lensmark/lansmark 오타 장애 차단).
- tsc·vitest 590(+2)·arch 0.

## 0.76.4 — 2026-06-17 · 기후 근거가 추천에 자연스럽게 (필지 추천 카드에 '🌤 이 땅 기후·추천 근거')
> UX 우선: 별도 위젯이 아니라 땅→추천→'왜'가 한 흐름으로.
- **자연스러운 노출** — `/api/recommend` 한 응답에 `climateEvidence`를 함께(fail-soft) → 프런트가 **추천 작물 바로 아래**에 '🌤 이 땅 기후 · 추천 근거'(연평균기온·적산온도·강수·겨울최저 등 실측 + 출처 '평년값 아님'·면책) 표시.
- 좌표 없으면(상위 줌)·기후 조회 실패해도 추천은 유지(기후 블록만 생략). 작물별 판정은 데이터기반 엔진 유지(날조 금지).
- tsc·vitest 588·arch 0. ⚠ 시각 확인은 배포 후. 전국 색지도는 KMA 격자 확보 후(HUMAN_GATE.md).

## 0.76.3 — 2026-06-17 · 기후 근거(필지 기후 프로필) 엔진 — 실측 GDD·연평균기온 (전국 색지도는 데이터 대기)
> nullschool풍 "기후 지도+근거" 요청. 사용자 관점 재정의: 위젯/애니메이션이 아니라 "왜 이 작물"의 근거가 진짜 필요.
- **기후 근거 엔진** — KMA 실측에서 **연평균기온·적산온도(GDD, 4~10월·기준10℃)** 직접 계산(`climateFromAsos`) + `core/climateEvidence.ts`가 평이한 '기후 근거' 문장으로(출처 '평년값 아님'·면책). 작물별 판정은 기존 `factors.climateFactors`(데이터 기반)에 위임 — **임계값 날조 금지**.
- **미구현(정직)** — 전국 색지도·작물별 'GDD 충분/부족'은 14지점 보간·임계값 날조가 되어 안 함 → KMA 격자 평년값 + 농진청 작물 base 확보 후(HUMAN_GATE.md 신청서 정리).
- featureMap `climate-evidence`(seam) 등록 · tsc·vitest **588(+5)**·arch 0. UI/API 노출은 다음 슬라이스(배포 후 사용자 검증).

## 0.76.2 — 2026-06-16 · 첫 화면 의도 묻기(온보딩) + 지도 타일 폴백 + 웰컴 모달 중앙정렬
> 사용자가 접속하면 목적부터 한 번 물어 그 흐름으로 안내(절충안: 건너뛰기 가능·가치-먼저). 차단망에서 지도가 빈 화면이 되던 것 완화.
- **의도 온보딩** — 웰컴 모달에 '무엇이 궁금하세요?' 칩(🌱귀농 준비·🌾작물 추천·💰소득 분석·👀둘러보기). 고르면 그 흐름으로 안내(귀농=자가진단·작물/소득=내 위치로 땅 고르기)+`localStorage('lansmark_intent')` 저장(개인화 토대). 막는 가입폼 아님. 진행 단계(1 땅→2 추천→3 소득)는 '이렇게 진행돼요' 로드맵으로 명시.
- **지도 타일 폴백** — VWorld 타일이 막히거나(키 도메인 불일치·통신사 IP 차단) 실패해 지도가 빈 배경만 남던 것(특히 확대 시) → `tileerror` 누적(>3) 시 OSM로 1회 자동 교체. 근본은 VWorld 키에 `lensmark.kr` 등록(HUMAN GATE).
- **웰컴 모달 제목 중앙정렬.** 프런트 단일파일 — tsc·vitest 583·arch 무관. 시각 확인은 배포 후 권장.
...
- **복구 안전(비가역)** — 관리자 가드(403/401/415 CSRF) + 'RESTORE' 타이핑(클라+서버 이중) + 복구 전 pre-restore 자동 스냅샷(2단 되돌리기) + id 경로주입 방어 + 재시작 유도(in-process 리로드 race 회피)
- **ops '🛟 백업/복구' 탭** — 상태·'지금 백업'·스냅샷별 '복구'(타이핑 확인) + DR 한계 정직 라벨. `/api/ops/backup`·`/status`·`/restore`
- **정직성** — 같은-DB 스냅샷은 운영 실수·논리 손상 복구용(재해≠보호). 진짜 DR=Layer2(GCP 관리형 PITR 7일+일일 스케줄·gcloud·HUMAN GATE)
- 검증: tsc · vitest(+18 backup) · arch(기능 38·엔드포인트 60) · ops 인라인 JS 구문 OK

## 0.74.6 — 2026-06-15 · 모바일 바텀시트 손가락 드래그(끌어서 작게/중간/전체)
> 0.74.5의 후속(②). 데스크톱 무영향(핸들 ≤600px만).
- **손가락 드래그** — 핸들을 끌면 시트가 따라오고(translateY 추적), 손 떼면 가장 가까운 단(작게/중간/전체)으로 스냅. 탭=순환은 유지
- 드래그 중 페이지 스크롤 차단·전환 끄고 추적→손 떼면 애니메이션. 터치↔클릭 합성 click 1회 무시(중복 방지)
- 인라인 JS 구문 OK·서빙 검증 · tsc/vitest 무관 · arch 0

## 0.74.5 — 2026-06-14 · 모바일 바텀시트 3단 스냅 + 지도 증발 방지 + 값넣기 토글
> 모바일 라이브 피드백. 데스크톱 무영향(≤600px만).
- **3단 스냅 바텀시트**(작게/중간/전체) — 핸들 탭=순환. 땅 탭 시 자동 '전체'→'중간'으로(지도 증발 방지), 전체에서도 지도 14vh 유지
- **값넣기 가시성** — 편집 패널에 '📝 내 값으로 조정' 헤더(면적·토양·판로 직접입력)
- 인라인 JS 구문 OK·서빙 검증 · arch 0 · 드래그 제스처는 후속

## 0.74.4 — 2026-06-14 · ops 추가기능 로드맵(스펙·프롬프트 보관) + 생육단계 가이드 스펙
> 미구현 기능 스펙을 ops에 프롬프트로 박제 — 제목 클릭 팝업. 첫 항목: 생육단계 가이드(준비만).
- **ops 로드맵 탭** — 미구현 기능 스펙을 '프롬프트'로 보관(제목 클릭→팝업·복사). 나중에 프롬프트만 주면 구현
- **생육단계 가이드 스펙**(기능 미구현) — 매일 동반자(리텐션+해자). 타임라인/스테퍼(바차트 아님)+수확진행바+병해 사진스트립. 실사진(농진청·농사로·NCPMS)만·AI진단 금지. 출처=HUMAN GATE
- 인라인 JS 구문 OK·서빙 검증 · arch 0
